...
| `src/encore/parser.py` | `parse_keyword()` (regex, default, money-adjacent path), `parse_llm()` (Claude; falls back to `parse_keyword` on any failure unless `strict=True`, which `parse-eval` and the agent's startup probe use so a model's name is never printed over keyword results), `evaluate()` (scores a parser against a labeled JSONL set). Everything routes through the pydantic `ReplyIntent` model. |
...
| `src/encore/webdata.py`, `src/encore/webhtml.py` | Pure builders for the evidence site: `build_results()` / `build_cliff()` turn `runs/eval.json` and the audit logs into `web/data/*.json`; `render_page()` fills `web/index.template.html` and raises on any `{{MARKER}}` it cannot fill. `encore web` does the I/O. |
| `web/` | The evidence site (GitHub Pages publishes it from `main`). `index.template.html`, `app.js`, `styles.css` are hand-written; `index.html`, `data/*.json`, `py/*.py` are generated by `encore web` and committed so the deploy is self-contained. `app.js` runs the repo's own `wall.py` in the browser through Pyodide. |
...
| `tests/` | One file per `src/encore/*` module, plus `test_cli.py`, `test_webdata.py`, `test_webhtml.py`, `test_board.py`, `test_agent.py`, `test_rails.py`, `test_sources.py`. 158 tests; `test_webhtml.py` also checks that `encore web --test-count`'s default equals the collected count. |
...
# --parser keyword|claude-haiku-4-5|claude-sonnet-5 picks the reply parser (default
# keyword). A model is probed once, strict, before the run; if it cannot answer
# the run refuses to start rather than print the model's name over keyword work.
```

```bash
uv run encore web [--eval-path runs/eval.json] [--audit-dir runs] [--test-count N]
# renders web/index.html from web/index.template.html, writes web/data/*.json and
# copies the Tier A modules to web/py/. Needs runs/eval.json (run `encore eval` first).
```
