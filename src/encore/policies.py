...
def legal_candidate_hours(failure_hour: int, cfg: WallConfig,
                          horizon_days: int = SEARCH_HORIZON_DAYS) -> list[int]:
    """Every hour within `horizon_days` after failure_hour that falls inside
    the wall's execution window.
...
class FixedSpread10:
    ...
    def __init__(self, cfg: WallConfig | None = None) -> None:
...
class RandomInHorizon:
    ...
    def __init__(self, rng: random.Random, cfg: WallConfig | None = None) -> None:
        self.rng = rng
        self._cfg = cfg or WallConfig()
    ...
    def propose(self, failed, state, now_hour):
        ...
        candidates = legal_candidate_hours(start - 1, self._cfg)
        if not candidates:
            return None
        return ProposedAction(ActionKind.RETRY, failed.customer_id, failed.cycle_id,
